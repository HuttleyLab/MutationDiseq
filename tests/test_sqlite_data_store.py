...
from mdeq import sqlite_data_store as sqlds
from mdeq import utils


def convert_compact_bootstrap(result) -> tuple:
    """takes compact bootstrap result object and converts
    it for storage in sqlitedb
    ...
    record = result.to_rich_dict(to_json=False)
    items = []
    for k, v in result.items():
        hyp = v.get_hypothesis_result("GSN", "GN")
        stats = {attr: getattr(hyp, attr) for attr in ("LR", "df", "pvalue")}
        data = {"data": compress(v.to_json().encode("utf8")), **stats}
        items.append([k, data])
    record["items"] = items
    return record, io.get_data_source(result.source)


def generic(result):
    source = Path(io.get_data_source(result))
    if suffixes := "".join(source.suffixes):
        source = source.name.replace(suffixes, ".json")
    else:
        source = f"{source}.json"
    return result, source


def migrate(path, converter):
    outpath = Path(f"{path.parent}/{path.stem}.sqlitedb")
    outpath = outpath.expanduser()
    writer = sqlds.sql_writer(
        outpath,
        if_exists="overwrite",
    )
    tdb_loader = io.load_db()
    tds = io.get_data_store(path)
    for m in track(tds, description=f"Converting complete for {path.name}"):
        rec = tdb_loader(m)
        rec, identifier = converter(rec)
        i = writer(rec, identifier=identifier)

    for m in track(tds.incomplete, f"Converting incomplete for {path.name}"):
        rec = tdb_loader(m)
        rec, identifier = converter(rec)
        i = writer.data_store.write(rec, identifier=identifier)

    if tds.logs:
        log = tds.logs[0].read()
        log = log.replace("tinydb", "sqlitedb")
        writer.data_store.add_log(log=log)

    writer.data_store.close()
    return outpath


DATA_TYPE_CONVERTERS = (("compact_bootstrap_result", convert_compact_bootstrap),)


def convert():
    path = Path(
        "/Users/gavin/repos/Honours2021/Kath/MutationDiseq/tests/data/fg_GSN_synthetic-lo_lo-300bp-1rep.tinydb"
    )
    converters = dict(DATA_TYPE_CONVERTERS)
    dstore = io.get_data_store(path, limit=1)
    dtype = utils.get_obj_type(dstore)
    converter = converters.get(dtype, generic)
    dest = migrate(path, converter)
